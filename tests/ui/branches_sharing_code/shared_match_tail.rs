...
// No lint: every arm is just the shared tail, so hoisting it would leave all arms empty. Linting
// here would only add noise (the proper fix is to drop the `match`, which we don't suggest).
fn all_arms_empty(mode: Mode) -> Result<u32, ()> {
    match mode {
        Mode::Kill => {
            Ok(1)
        },
